...
# =============================================================================
select_year_site <- c(NULL)  # <- задать нужные или NULL для всех 2023_33
...
# ===== ПАРАМЕТРЫ =====
rds_path   <- "/home/npwc/GIT/TLC MarkUp System data/Models/SSL_presence_binary_256_winter_2026-03-18_acc_0949_ep_29"
Preddir    <- "/home/npwc/adata8tb/SSL_DB_Tiles"
file_size  <- 2000
batch_size <- 256L
trgt_size  <- 256L
threshold  <- 0.8
...
if (!is.null(select_year_site)) {
  patt <- paste0("(", paste(select_year_site, collapse = "|"), ")_Tiles$")
  listsites <- listsites[grepl(patt, basename(listsites))]
}
